...
const Register: React.FC = () => {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      if (clientData.status !== "success") {
        throw new Error(formatError(clientData.error || clientData.message || "Client registration failed"));
      }
      ...
      if (membershipData.status !== "success") {
        throw new Error(formatError(membershipData.error || membershipData.message || "Membership creation failed"));
      }
  ...
  const nextStep = () => {
    if (step === 1) {
      setStep(2);
    } else if (step === 2 && selectedMemberships.length > 0) {
      setStep(3);
    }
  };
  ...
  const canProceedToPlans = selectedMemberships.length > 0;
  const canSubmit = selectedPlans.length > 0;
        ...
        >
                            ...
                            >
                              ...
                              <div className="space-y-2">
                                <div className="flex items-center text-sm">
                                  <CurrencyRupeeIcon className="w-4 h-4 text-green-400 mr-1" />
                                ...
                                <div className="flex items-center text-sm">
                                  <DevicePhoneMobileIcon className="w-4 h-4 text-blue-400 mr-1" />
                                  <span className="text-white">{plan.Quota.toLocaleString()} SMS</span>
                                </div>
                              </div>
                            </motion.div>
                          ))}
                          ...
                          {whatsappPlans.map((plan) => (
                            ...
                            >
                              ...
                              <div className="space-y-2">
                                ...
                                <div className="flex items-center text-sm">
                                  <ChatBubbleLeftRightIcon className="w-4 h-4 text-green-400 mr-1" />
                                  <span className="text-white">{plan.Quota.toLocaleString()} messages</span>
                                </div>
                              </div>
                            </motion.div>
                          ))}
          ...
          <div className="flex justify-between mt-8">
            ...
            {step < 3 ? (
              <button
                onClick={nextStep}
                disabled={step === 2 && !canProceedToPlans}
                className="px-6 py-3 bg-gradient-to-r from-cyan-500 to-purple-500 text-white rounded-xl hover:from-cyan-600 hover:to-purple-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Next
              </button>
            ) : (
